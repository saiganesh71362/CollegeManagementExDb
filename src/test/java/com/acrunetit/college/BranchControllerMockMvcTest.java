...
@SpringBootTest(classes = { BranchControllerMockMvcTest.class })
class BranchControllerMockMvcTest {
	...
	@Test
	@Order(1)
	void test_getAllBranches() throws Exception {
		...
		mockMvc.perform(get("/branch/allBranches")).andExpect(status().isOk()).andDo(print());

	}

	@Test
	@Order(2)
	void test_getBranchById() throws Exception {
		College college1 = new College();
		college1.setCollegeId(1);
		college1.setCollegeName("Green Valley University");
		college1.setCollegeAddress("123 Green Valley St, Springfield");
		...
		this.mockMvc.perform(get("/branch/getBranchById/{branchId}",1))
